...
- **Backend Stack**:
  - **Authentication**: Firebase Auth (Email/Google) - *Enhanced with dedicated Forgot Password flow and strong validations.*
  - **Database**: Firebase Cloud Firestore
  - **Storage**: (Phase 3) For user photos.
- **AI Capability**: Firebase GenAI SDK (Gemini 1.5/2.0) with CSV Grounding and specialized "Period Mode" logic.
- **External Dependencies**: OpenFoodFacts API, ML Kit, Retrofit, Coroutines.
...
Built a robust authentication and profile management system.

- Secure Login/Signup with Firebase.
- **Refined Security**: Strong password validation (Upper/Lower/Digit/Special) and phone validation.
- **Dedicated Reset Flow**: Custom `ForgotPasswordActivity` with matching UI.
...
- Firebase GenAI SDK / Gemini 2.0 Flash integration.
- Adaptive logic based on exercise intensity and weight trends.
- Festival-aware meal planning and **Period Mode** support.
