...
private fun checkPosture(event: SensorEvent) {
    ...
    // High Y-value with low X-value indicates the phone is tilted 
    // in a way that suggests the user is looking down/slouching.
...
- **Auto-Logging**: Once a product is found via scan, it is automatically saved to the user's `foodLogs` in Firestore without manual typing.
