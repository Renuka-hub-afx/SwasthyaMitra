...
```kotlin
// From WaterReminderManager.kt
fun scheduleReminders(wakeTime: String, sleepTime: String) {
    val (wakeH, wakeM) = parseTime(wakeTime)
    val activeHours = calculateActiveHours(wakeH, sleepH)
    val reminderCount = (activeHours * 60 / 120).toInt() // Every 2 hours
    
    // Iteratively schedule alarms for each block
    for (i in 0 until reminderCount) {
        scheduleAlarm(triggerMillis, requestCode, intervalMillis)
    }
}
```
